...
cmd:option('-alg_name', 'LstmAttCtrlGradNoiseDontSelectMultEarlyStopLinSchedule224Pool5', 'algorithm name')
...
cmd:text('DATA')
cmd:option('-split', 'test-dev2015', 'option: [val2014|test2015|test-dev2015]')
cmd:option('-vqa_dir', './data/VQA_prepro/data_train-val_test-dev','vqa data directory')
cmd:option('-feat_dir', './data/vqa_VGG16Conv_pool5_448/feat_448x448', 'vqa feature directory')
...
tab_multhop_compute_loss = {}
tab_multhop_stop_timing = {}
print(('multhop early stop timing, nHop: %d, max_epochs: %d'):format(nHop, opt.max_epochs))
tab_multhop_stop_timing[1] = 1000
tab_multhop_stop_timing[2] = 35
tab_multhop_stop_timing[3] = 25
tab_multhop_stop_timing[4] = 20
tab_multhop_stop_timing[5] = 18
tab_multhop_stop_timing[6] = 16
tab_multhop_stop_timing[7] = 16
tab_multhop_stop_timing[8] = 16
tab_multhop_stop_timing[9] = 16
tab_multhop_stop_timing[10] = 1000
print('multhop_stop_timing')
print(tab_multhop_stop_timing)
...
for h=1,nHop+2 do
   tab_multhop_compute_loss[h] = true
   tab_train_acc[h] = 0
...
function feval(step_t)
   ...
   for h=nHop,1,-1 do
      ...
      if tab_multhop_compute_loss[h] == false then
         dpred:mul(0) 
      end      
...
for it = 1, total_iter do
   ...
   if (it % iter_epoch == 0 and epoch % opt.test_interval == 0) or it == total_iter then
      ...
      if opt.split == 'val2014' then
         ...
         for h=1,nHop+2 do
            if tab_multhop_stop_timing[h] <= epoch then
               tab_multhop_compute_loss[h] = false
            end
            local compute_loss_flag = 1
            if tab_multhop_compute_loss[h] == false then
               compute_loss_flag = 0
            end
            ...
            tab_testlog[string.format('cmploss_%02d', h)] = compute_loss_flag
